...
class SerialCH340App:
    def __init__(self, master):
        ...
        self.running = False
        self.stream_to_send = "V2-006E-2D4E-000E-000E-000E-000E-000E-000E-000E-@CF0E"

        # Área de log principal
        self.text_area = scrolledtext.ScrolledText(master, width=80, height=15)
        self.text_area.pack(pady=(10, 5))

        # Área para mostrar apenas o INDEX
        self.index_info_box = tk.Text(master, height=2, width=80, state=tk.DISABLED, bg="#f0f0f0", fg="blue")
        self.index_info_box.pack(pady=(0, 10))
    ...
    def check_combined_packets(self, text):
        # Encontra todos os pacotes r: com 4 caracteres hexadecimais
        r_matches = re.findall(r"r:([0-9A-Fa-f]{4})", text)
        if len(r_matches) >= 2:
            index_hex = r_matches[1]  # segundo pacote r: = quarto pacote total
            bin_value = bin(int(index_hex, 16))[2:].zfill(16)  # binário com padding
            extracted_bits = bin_value[2:10]  # ignora os 2 primeiros, pega os próximos 8
            index_decimal = int(extracted_bits, 2)

            info = f"Index: {index_decimal} (extraído de {index_hex} -> {bin_value})"
            self.index_info_box.config(state=tk.NORMAL)
            self.index_info_box.delete(1.0, tk.END)
            self.index_info_box.insert(tk.END, info)
            self.index_info_box.config(state=tk.DISABLED)
